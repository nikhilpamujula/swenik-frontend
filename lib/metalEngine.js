...
async function run({ send = false, test = false } = {}) {
  ...
  // Picker — the EXECUTABLE view (8 Aug). Indian retail can BUY any ETF but
  // cannot hold an ETF short overnight (SLB is effectively dead for ETFs), so
  // the daily-use question is "which one, and is now a good time" — not
  // "short the rich one". Live iNAV rows only; DATA_SUSPECT excluded. Band is
  // judged on the CHEAPEST premium: if even the cheapest entry is rich, the
  // whole complex is rich.
  const PICKER_BAND_DEFAULTS = {
    silver: { cheapPct: 1.5, richPct: 3.0 },
    gold: { cheapPct: 0.5, richPct: 1.5 },
  };
  const picker = {};
  for (const metal of ['silver', 'gold']) {
    const live = etfs
      .filter((e) => e.metal === metal && e.premiumPct !== null
        && (e.inavSource === 'nse-live' || e.inavSource === 'nse-quote')
        && Math.abs(e.premiumPct) <= 30)
      .sort((a, b) => a.premiumPct - b.premiumPct);
    if (!live.length) continue;
    const best = live[0], worst = live[live.length - 1];
    const bands = (settings.pickerBands || {})[metal] || PICKER_BAND_DEFAULTS[metal];
    const band = best.premiumPct <= bands.cheapPct ? 'CHEAP'
      : best.premiumPct >= bands.richPct ? 'RICH' : 'NORMAL';
    picker[metal] = {
      band, bands,
      best: { symbol: best.symbol, premiumPct: best.premiumPct, ltp: best.ltp, inav: best.inav },
      worst: { symbol: worst.symbol, premiumPct: worst.premiumPct },
      saveBps: live.length > 1 ? Math.round((worst.premiumPct - best.premiumPct) * 100) : 0,
      rows: live.map((e) => ({ symbol: e.symbol, metal: e.metal, ltp: e.ltp, inav: e.inav,
        premiumPct: e.premiumPct, inavSource: e.inavSource })),
      note: band === 'CHEAP'
        ? `complex premium compressed — good time to add ${metal} via ${best.symbol}`
        : band === 'RICH'
          ? `complex premium stretched — avoid adding ${metal}; holders can consider trimming the richest (${worst.symbol})`
          : `if adding ${metal} today, ${best.symbol} is the cheapest door in`,
    };
  }
