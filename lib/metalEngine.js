...
async function run({ send = false, test = false } = {}) {
  ...
  const missing = etfs.filter(e => e.inav == null).slice(0, 8);
  for (const e of missing) {
    const v = await getNseInav(e.symbol, nseCookie, errors);
    if (v) { e.inav = v; e.inavSource = 'nse-quote'; }
    await new Promise((r) => setTimeout(r, 250));
  }
  // Stage 3: AMFI EOD NAV, token-matched — approximate, labelled.
  for (const e of etfs) {
    if (e.inav == null) {
      const m = amfiInavMatch(e, amfi);
      if (m) { e.inav = m.nav; e.inavSource = 'amfi-eod'; }
    }
  }

  // Now compute the signals off whatever iNAV each ETF ended up with.
  for (const e of etfs) {
    e.premiumPct = e.ltp && e.inav ? +(((e.ltp - e.inav) / e.inav) * 100).toFixed(2) : null;
    e.indiaVsGlobalPct = null;
    if (e.gramsPerUnit && e.inav) {
      const inrPerGram = e.inav / e.gramsPerUnit;
      const landedPerGram = e.metal === 'silver'
        ? (landed.silverInrPerKg ? landed.silverInrPerKg / 1000 : null)
        : (landed.goldInrPer10g ? landed.goldInrPer10g / 10 : null);
      if (landedPerGram) e.indiaVsGlobalPct = +(((inrPerGram - landedPerGram) / landedPerGram) * 100).toFixed(2);
    }
    e.approxIndiaComparison = !!e.gramsPerUnit;
  }
  etfs.sort((a, b) => Math.abs(b.premiumPct ?? 0) - Math.abs(a.premiumPct ?? 0));

  // VERDICTS — the page's conclusion layer. Spread/convergence framing (his
  // agreed understanding, preserved): the structural India premium is duty+GST;
  // only deviations beyond each ETF's threshold are called actionable. The
  // hedge suggestion always names MCX as the clean domestic leg; the Binance
  // perp leg is informational with its carry cost — FEMA-grey, his risk call
  // (standing caveat, do not drop).
  const conclusions = [];
  for (const e of etfs) {
    if (e.premiumPct === null) { e.verdict = { action: 'NO_DATA', why: 'no iNAV from any source this run' }; continue; }
    const approx = e.inavSource === 'amfi-eod' ? ' (vs EOD NAV — approximate)' : '';
    const thr = e.thresholdPct ?? 2;
    const perp = (binance || []).find((b) => (e.metal === 'gold' ? /XAU/ : /XAG/).test(b.symbol || ''));
    const carry = perp && perp.fundingDailyPct != null ? `${perp.fundingDailyPct}%/day` : null;
    if (e.premiumPct >= thr) {
      e.verdict = { action: 'SHORT_ETF',
        why: `${e.premiumPct}% ABOVE iNAV${approx} — rich vs threshold ${thr}%${e.indiaVsGlobalPct != null ? ` · ${e.indiaVsGlobalPct > 0 ? '+' : ''}${e.indiaVsGlobalPct}% vs global landed` : ''}`,
        hedge: `hedge: LONG MCX ${e.metal} futures (clean domestic)${carry ? ` · perp alt carry ${carry} (FEMA grey — your call)` : ''}` };
    } else if (e.premiumPct <= -thr) {
      e.verdict = { action: 'BUY_ETF',
        why: `${Math.abs(e.premiumPct)}% BELOW iNAV${approx} — cheap vs threshold ${thr}%`,
        hedge: `hedge: SHORT MCX ${e.metal} futures (clean domestic)${carry ? ` · perp alt carry ${carry} (FEMA grey — your call)` : ''}` };
    } else {
      e.verdict = { action: 'HOLD', why: `${e.premiumPct > 0 ? '+' : ''}${e.premiumPct}% vs iNAV${approx} — inside the normal ±${thr}% band, no edge` };
    }
    if (e.verdict.action === 'SHORT_ETF' || e.verdict.action === 'BUY_ETF') {
      conclusions.push({ symbol: e.symbol, metal: e.metal, action: e.verdict.action,
        premiumPct: e.premiumPct, inavSource: e.inavSource, why: e.verdict.why, hedge: e.verdict.hedge });
    }
  }
